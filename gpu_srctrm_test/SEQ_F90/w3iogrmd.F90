...
!
          ...
          NK2    = NK + 2
          NSPEC  = NK * NTH
!$ACC ENTER DATA CREATE(NK, NSPEC, NTH)
!$ACC UPDATE DEVICE(NK, NSPEC, NTH)
!
...
!$ACC ENTER DATA COPYIN(ICESCALES(1:4), DTMAX, DTMIN, DMIN)
!
        END IF
      ...
      IF ( WRITE ) THEN
        ...
        ELSE
          ...
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR)                     &
               ...
               XFR, FR1, SIG, SIG2, DSIP, DSII, DDEN, DDEN2, FTE,     &
               FTF, FTWN, FTTR, FTWL, FACTI1, FACTI2, FACHFA, FACHFE
!$ACC ENTER DATA CREATE(DDEN,DDEN2,ES2,ESIN,ECOS,DTH,SIG,SIG2) &
!$ACC            CREATE(EC2,FACHFA,FACHFE,FACTI1,FACTI2,FTF,FTE,ESC)
!$ACC UPDATE DEVICE (DDEN,DDEN2,ES2,ESIN,ECOS,DTH,SIG,SIG2, &
!$ACC                EC2,FACHFA,FACHFE,FACTI1,FACTI2,FTF,FTE,ESC)
        END IF
      ...
      IF ( WRITE ) THEN
        ...
        ELSE
          ...
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR)                     &
                FACP, XREL, XFLT, FXFM, FXPM, XFT, XFC, FACSD, FHMAX, &
                FFACBERG, DELAB, FWTABLE
!$ACC ENTER DATA CREATE(XREL,XFLT,XFC, XFT,FACP, FWTABLE)
!$ACC UPDATE DEVICE(XREL,XFLT,XFC,XFT,FACP,FWTABLE,DELAB)
        END IF
!
...
!$ACC ENTER DATA CREATE(SLNC1, FSPM, FSHF)
!$ACC UPDATE DEVICE(SLNC1, FSPM, FSHF)
        END IF
!
...
!read on CPU. 
!$ACC ENTER DATA CREATE(TAUT, TAUHFT, TAUHFT2, QBI, IKTAB, DCKI) &
!$ACC            CREATE(SSDSC, SSDSISO, SSDSBR,SSDSBRFDF, CUMULW)&
!$ACC            CREATE(SATINDICES, SATWEIGHTS, SSDSDTH, SSDSP)  &
!$ACC            CREATE(ZZ0RAT, ZZALP, SSINTHP, BBETA, SSWELLF)  &
!$ACC            CREATE(ZZWND, AALPHA, TTAUWSHELTER,DELUST,DELALP) &
!$ACC            CREATE(SSDSBM, SSTXFTFTAIL, SSTXFTWN,DELTAIL)     &
!$ACC            CREATE(WWNMEANP, WWNMEANPTAIL, FFXFM, FFXPM)    &
!$ACC            CREATE(FFXFA,SSINBR,DELU,DELTAUW )
!$ACC UPDATE DEVICE(TAUT, TAUHFT, TAUHFT2, QBI, IKTAB, DCKI, &
!$ACC               SSDSC, SSDSISO, SSDSBR,SSDSBRFDF, CUMULW,&
!$ACC               SATINDICES, SATWEIGHTS, SSDSDTH, SSDSP,  &
!$ACC               ZZ0RAT, ZZALP, SSINTHP, BBETA, SSWELLF,  &
!$ACC               ZZWND, AALPHA, TTAUWSHELTER , &
!$ACC               SSDSBM, SSTXFTFTAIL, SSTXFTWN,     &
!$ACC               WWNMEANP, WWNMEANPTAIL, FFXFM, FFXPM,    &
!$ACC               FFXFA,SSINBR, DELU,DELTAUW,DELTAIL,DELUST,DELALP)
