...
!$ACC ENTER DATA COPYIN(ICESCALES, DTMAX, DTMIN, DMIN)
!
        END IF
...
!$ACC            COPYIN(EC2, FACHFE, FACTI1, FACTI2, FTF, FTE)
        END IF
      ...
      IF ( WRITE ) THEN
        ...
        ELSE
          READ (NDSM,END=801,ERR=802,IOSTAT=IERR)                     &
                FACP, XREL, XFLT, FXFM, FXPM, XFT, XFC, FACSD, FHMAX, &
                FFACBERG, DELAB, FWTABLE
!$ACC ENTER DATA COPYIN(XREL,XFLT,XFC, XFT,FACP, FWTABLE)
        END IF
!
...
!$ACC            COPYIN(WWNMEANP, WWNMEANPTAIL) 
        END IF
!
